...
export default async function TabLayout({ children }: LayoutProps<"/tab">) {
  // Real Premier League calendar - same source The Tab's own pot rules use
  // to decide seasonOver (lib/tab.ts), not the Draft section's separate,
  ...
  return (
    <main className="flex-1 w-full max-w-6xl mx-auto px-4 sm:px-6 pt-3 sm:pt-4 pb-4 sm:pb-6">
      {/* No right-side action button - The Tab is read-only, so there's
